...
use crate::{
    ...
    utils::reader::{read_address, read_curve_compressed, read_field_be, read_h256, read_u256},
};
...
pub fn decode_ecdsa_subquery(mut reader: impl Read) -> Result<ECDSAComponentNativeInput> {
    let pubkey_x = read_field_be(&mut reader)?;
    let pubkey_y = read_field_be(&mut reader)?;
