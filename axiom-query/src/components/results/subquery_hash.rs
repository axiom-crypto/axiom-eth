...
pub fn get_subquery_hash<F: Field>(
    ...
    let res = keccak.keccak_var_len(ctx, encoded_key, *encoded.len(), min_len);
    res
}
...
pub fn transform_subquery_key_to_bytes<F: Field>(
    ...
    let last_one = enabled_types.iter().rposition(|&x| x).expect("No subqueries enabled");

    let gate = range.gate();
    let type_indicator = gate.idx_to_indicator(ctx, subquery_type, last_one + 1);
