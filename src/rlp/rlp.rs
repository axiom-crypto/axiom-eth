...
use crate::rlp::rlc::{log2, RlcChip, RlcTrace};
...
pub fn witness_subarray_from_idxs<'a, F: Field>(
    ...
    let ret_vals = start_idx.zip(end_idx).zip(vec_val).map(|((si, ei), vv)| {
...
impl<F: Field> RlpArrayChip<F> {
    pub fn configure(
        ...
        let rlc =
            RlcChip::configure(meta, num_basic_chips, num_chips_fixed, challenge_id, context_id);
    ...
    pub fn parse_rlp_field_prefix(
        ...
        let field_len = range.gate.sub(ctx, &Existing(prefix), &Constant(F::from(128u64)))?;
        let len_len = range.gate.sub(ctx, &Existing(prefix), &Constant(F::from(183u64)))?;
        ...
        let len_len_mid = range.gate.mul(ctx, &Existing(&len_len), &Existing(&is_big))?;

        let len_assigned = range.gate.assign_region_smart(
            ctx,
            vec![
                Witness(
                    len_len_mid.value().copied()
        ...
        Ok(RlpFieldPrefixParsed { is_literal, is_big, next_len, len_len, prefix: prefix.clone() })
    }
    ...
    pub fn parse_rlp_array_prefix(
        ...
        let len_len_final = range.gate.mul(ctx, &Existing(&len_len), &Existing(&is_big))?;
    ...
    pub fn decompose_rlp_array(
        ...
        let (len_cells, len_byte_val) =
            self.parse_rlp_len(ctx, range, &rlp_array, &len_len, max_len_len)?;
        ...
        for idx in 0..num_fields {
            ...
            range.check_less_than_safe(
                ctx,
                &len_len,
                max_rlp_len_len(max_field_lens[idx]) + 1,
            ...
            let field_len_cells = witness_subarray_from_idxs(
                ...
                prefix_idxs[idx].value().copied()
                    + Value::known(F::from(1))
                    + len_len.value().copied(),
        ...
        let one_vec = self.range.gate.assign_region_smart(
            ctx,
            vec![Constant(F::from(1))],
            vec![],
            vec![],
            vec![],
        )?;
        ...
        let mut rlc_and_len_inputs =
            vec![(prefix.clone(), one.clone()), (len_rlc.rlc_val.clone(), len_rlc.rlc_len.clone())];
        for idx in 0..num_fields {
        ...
        let max_array_assigned = range.gate.assign_region_smart(
            ...
            vec![
                Witness(
                    Value::known(F::from(max_array_len as u64)) - rlp_rlc.rlc_len.value().copied(),
                ),
...
#[cfg(test)]
mod tests {
    use crate::rlp::{rlc::log2, rlp::RlpTestCircuit};
