...
fn test_mpt_circuit<F: Field>(
    ...
    let mpt_witness = mpt.parse_mpt_inclusion_phase0(ctx, &mut keccak, mpt_proof);
    ...
    let circuit = KeccakCircuitBuilder::new(
        ...
        move |builder: &mut RlcThreadBuilder<F>,
            ...
            let mpt = EthChip::new(rlp, Some(keccak_rlcs));
...
pub fn test_mock_mpt_inclusion_fixed() {
    ...
    let k = params.degree;
    let input = mpt_input_storage("scripts/input_gen/default_storage_pf.json", false, 5, 32, None); // depth = max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();

    let input =
        mpt_input_storage("scripts/input_gen/default_storage_pf.json", false, 5, 32, Some(32)); // depth != max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
...
pub fn test_mpt_noninclusion_branch_fixed() {
    ...
    let k = params.degree;
    let input =
        mpt_input_storage("scripts/input_gen/noninclusion_branch_pf.json", true, 5, 32, None);
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
    let input =
        mpt_input_storage("scripts/input_gen/noninclusion_branch_pf.json", true, 5, 32, Some(32));
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
...
pub fn test_mpt_noninclusion_extension_fixed() {
    ...
    let k = params.degree;
    let input =
        mpt_input_storage("scripts/input_gen/noninclusion_extension_pf.json", true, 6, 32, None); // require depth < max_depth
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
    let input = mpt_input_storage(
...
pub fn test_mpt_empty_root() {
    let params = EthConfigParams::from_path("configs/tests/mpt.json");
    let k = params.degree;
    let input = mpt_input_storage("scripts/input_gen/empty_storage_pf.json", true, 5, 32, None);
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
    let input = mpt_input_storage("scripts/input_gen/empty_storage_pf.json", true, 5, 32, Some(32));
    let circuit = test_mpt_circuit(k, RlcThreadBuilder::<Fr>::mock(), input);
    MockProver::run(k, &circuit, vec![]).unwrap().assert_satisfied();
}
