...
impl<F: Field> EthBlockHeaderTraceWitness<F> {
    pub fn get_block_num(&self, ctx: &mut Context<F>, gate: &GateChip<F>) -> Vec<AssignedValue<F>> {
        let block_num_bytes = &self.get_number().field_cells;
        let block_num_len = self.get_number().field_len;
        bytes_be_var_to_fixed(ctx, gate, block_num_bytes, block_num_len, 4)
    }
    pub fn get_block_hash_hi_lo(
        &self,
        ctx: &mut Context<F>,
        gate: &GateChip<F>,
    ) -> Vec<AssignedValue<F>> {
        bytes_be_to_u128(ctx, gate, &self.block_hash)
    }
...
pub trait EthBlockHeaderChip<F: Field> {
    ...
    fn decompose_block_header_phase0(
        ...
        block_header_rlp: &[AssignedValue<F>],
        network: Network,
...
impl<'chip, F: Field> EthBlockHeaderChip<F> for EthChip<'chip, F> {
    fn decompose_block_header_phase0(
        ...
        let rlp_witness =
            self.rlp().decompose_rlp_array_phase0(ctx, block_header.to_vec(), max_field_lens, true); // `is_variable_len = true` because RLP can have between 15 to 17 fields, depending on which EIPs are active at that block
