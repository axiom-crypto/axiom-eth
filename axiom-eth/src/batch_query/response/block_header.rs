...
use crate::{
    ...
    block_header::{
        get_block_header_rlp_max_lens, EthBlockHeaderChip, EthBlockHeaderTraceWitness,
...
pub struct BlockHeader<F: ScalarField> {
    as_list: [(FixedByteArray<F>, Option<AssignedValue<F>>); NUM_BLOCK_HEADER_FIELDS],
...
impl EthPreCircuit for MultiBlockCircuit {
    fn create(
        ...
        // verify mmr
        let mmr_list_len = ctx.load_witness(gate.get_field_element(self.mmr_list_len as u64));
        let mmr_bits = gate.num_to_bits(ctx, mmr_list_len, MMR_MAX_NUM_PEAKS); // implicitly range checks that mmr_list_len < 2^MMR_MAX_NUM_PEAKS

        let mmr = self
            .mmr
            .into_iter()
            .map(|peak| FixedByteArray::new(ctx, &range, peak.as_bytes()))
            .collect_vec();
        // check that mmr peaks agree with mmr_bits
        for (bit, peak) in mmr_bits.iter().zip_eq(mmr.iter()) {
            let no_peak = is_zero_vec(ctx, gate, &peak.0);
            let is_peak = gate.not(ctx, no_peak);
            ctx.constrain_equal(&is_peak, bit);
        }
        // verify mmr proofs
        block_responses.iter().zip(self.mmr_proofs).zip_eq(not_empty.clone()).for_each(
        ...
        let mut is_leading = Constant(Fr::one());
