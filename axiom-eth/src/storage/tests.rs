...
fn get_test_circuit_detailed_tether(
    ...
    let provider =
        Provider::new_client(&provider_url, 3, 500).expect("could not instantiate HTTP Provider");
    let addr;
    let block_number;
    match network {
        Network::Mainnet => {
...
pub fn test_mock_small_val() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let slot = H256(bytes);
    let input = get_test_circuit_detailed_tether(Network::Mainnet, vec![slot]);
    let circuit = input.create_circuit(RlcThreadBuilder::mock(), None);
    MockProver::run(k, &circuit, vec![circuit.instance()]).unwrap().assert_satisfied();
    Ok(())
}
